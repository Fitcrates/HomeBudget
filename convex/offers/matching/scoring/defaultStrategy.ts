...
export const defaultScoringStrategy: ScoringStrategy = {
  ...
  score(input) {
    ...
    const factors = [
      { key: "segment_match", value: clamp(segmentStrength / 100, 0, 1), weight: 35 },
      { key: "category_spend", value: clamp(matchedSpend / 150000, 0, 1), weight: 25 },
      {
        key: "savings_value",
      ...
      },
      {
        ...
        value: input.offer.revenueModel === "cps"
          ? clamp((input.offer.commissionRate ?? 0) / 10, 0, 1)
          : clamp((input.offer.commissionAmount ?? 0) / 1000, 0, 1),
        weight: 10,
      },
      { key: "source_priority", value: clamp(input.source.priorityBoost / 20, 0, 1), weight: 10 },
      { key: "offer_weight", value: clamp(input.offer.weight / 20, 0, 1), weight: 5 },
    ];
