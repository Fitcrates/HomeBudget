...
export function matchHome(text: string, combinedContext: string, issuers: IssuerFlags, categoriesArray: any[]): CategoryResolution | null {
  ...
  if (isHomeIssuer || has(text, /\b(donicz|ziemia|nawoz|zarowka|mebel|poduszka|posciel|pojemnik|narzedz|wiert|mlotek|farba|pedzel|balkon|ogrod|garnek|patelnia|talerz|dekoracja|koc\b|przescieradlo|miska|deska)\b/i)) {
    if (has(combinedContext, /\b(donicz|ziemia|nawoz|balkon|ogrod|roslin|krzew|kwiat|sadzonk|nasion|traw|sekator|konewk)\b/i)) return resolve(CATEGORY.HOME, SUB.ogrod, categoriesArray);
    if (has(combinedContext, /\b(narzedz|wiert|mlotek|farba|pedzel|remont|kabel|silikon|plytka|szpachl|gwozdz|srub|wkret|tasma malarska)\b/i)) return resolve(CATEGORY.HOME, SUB.remonty, categoriesArray);
    if (has(combinedContext, /\b(garnek|patelni|miska|talerz|kubek|noz kuchenny|deska do krojen|lyzka|widelec|szklank|termos|foremka|pojemnik na zywnosc)\b/i)) return resolve(CATEGORY.HOME, SUB.kuchnia, categoriesArray);
    if (has(combinedContext, /\b(ramka|swiecznik|plakat|poduszka dekoracyjna|zaslona|firanka|obraz|wazon|dywan)\b/i)) return resolve(CATEGORY.HOME, SUB.dekoracje, categoriesArray);
    if (has(combinedContext, /\b(mebel|krzeslo|stol|komoda|lampa|polka|szafa|lozko|materac|fotel|biurko|zarowk|gniazdko|kontakt|posciel|przescieradlo|koc\b|recznik)\b/i)) return resolve(CATEGORY.HOME, SUB.wyposazenie, categoriesArray);
...
export function matchHomeStandalone(text: string, categoriesArray: any[]): CategoryResolution | null {
  ...
  if (has(text, /\b(garnek|patelni|miska|talerz|kubek|noz kuchenn|deska do krojen|lyzka|widelec|szklank|termos|foremka|pojemnik na zywnosc)\b/i)) return resolve(CATEGORY.HOME, SUB.kuchnia, categoriesArray);
  ...
  if (has(text, /\b(mebel|krzeslo|stol|komoda|lampa|polka|szafa|lozko|materac|fotel|biurko|zarowk|gniazdko|kontakt|posciel|przescieradlo|koc\b|recznik)\b/i)) return resolve(CATEGORY.HOME, SUB.wyposazenie, categoriesArray);
