...
export function matchFamily(text: string, combinedContext: string, issuers: IssuerFlags, categoriesArray: any[]): CategoryResolution | null {
  ...
  if (isToyIssuer || has(combinedContext, /\b(zabawka|lalka|klocki|lego|pluszak|gra planszowa dla dzieci|puzzle|hot wheels|playmobil)\b/i)) return resolve(CATEGORY.KIDS, SUB.zabawki, categoriesArray);
  if (has(combinedContext, /\b(pieluch|chusteczki dla dzieci|mleko modyfikowane|smoczek|butelka dla niemowlat|kaszka|body dzieciece|wozek|fotelik)\b/i)) {
    if (has(combinedContext, /\b(pieluch|pampers|chusteczki|krem na odparzenia)\b/i)) return resolve(CATEGORY.KIDS, SUB.pieluchy, categoriesArray);
  ...
  if (isPetIssuer || has(text, /\b(karma|zwirek|weteryn|drapak|smycz|obroza|kuweta|miska|przysmak dla psa|przysmak dla kota)\b/i)) {
    ...
    if (has(combinedContext, /\b(karma|saszetka|puszka|sucha karma|whiskas|felix|royal canin|purina|hills|pedigree)\b/i)) return resolve(CATEGORY.PETS, SUB.karma, categoriesArray);
    if (has(combinedContext, /\b(grooming|strzyzenie psa|kapiel psa|kąpiel psa|pielegnacja psa|szampon dla psa)\b/i)) return resolve(CATEGORY.PETS, SUB.grooming, categoriesArray);
...
export function matchFamilyStandalone(text: string, categoriesArray: any[]): CategoryResolution | null {
  if (has(text, /\b(zabawk|lalka|klocki|lego|pluszak|gra planszowa|puzzle|hot wheels|playmobil)\b/i)) return resolve(CATEGORY.KIDS, SUB.zabawki, categoriesArray);
  ...
  if (has(text, /\b(karma|saszetka|puszka|sucha karma|whiskas|felix|royal canin|purina|hills|pedigree)\b/i)) return resolve(CATEGORY.PETS, SUB.karma, categoriesArray);
