...
export function matchHealth(text: string, combinedContext: string, issuers: IssuerFlags, categoriesArray: any[]): CategoryResolution | null {
  ...
  if (isPharmacyIssuer || has(text, /\b(lek|tablet|tabl|syrop|kaps|suplement|wit|vit|masc|krem lecz|plaster|termometr)\b/i)) return resolve(CATEGORY.HEALTH, SUB.apteka, categoriesArray);
  ...
  if (has(combinedContext, /\b(serum|krem do twarzy|tonik|peeling do twarzy|maska do twarzy)\b/i)) return resolve(CATEGORY.HEALTH, SUB.twarz, categoriesArray);
...
export function matchHealthStandalone(text: string, categoriesArray: any[]): CategoryResolution | null {
  if (has(text, /\b(lek\b|tablet|tabl|syrop|kaps|suplement|wit\b|vit\b|masc|krem lecz|plaster|termometr|ibuprom|paracetamol|apap|rutinoscorbin|vitamina)\b/i)) return resolve(CATEGORY.HEALTH, SUB.apteka, categoriesArray);
  if (has(text, /\b(serum|krem do twarzy|tonik|peeling do twarzy|maska do twarzy|retinol|kwas hialuro)\b/i)) return resolve(CATEGORY.HEALTH, SUB.twarz, categoriesArray);
  return null;
}
