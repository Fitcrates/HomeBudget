    ...
    <div className="space-y-2.5">
      {data.map((bar) => (
        <div key={bar.period} className="flex items-center justify-between gap-2">
          <span className="text-[10px] font-bold text-[#b89b87] uppercase tracking-wider w-16 flex-shrink-0 text-left drop-shadow-[0_1px_1px_rgba(255,255,255,0.8)]">{bar.period}</span>
          ...
          <span className="text-xs font-bold text-[#3e2815] w-12 flex-shrink-0 text-left drop-shadow-sm">{fmt(bar.total)}</span>
        </div>
      ))}
    </div>
  );
